...
const useInvoice = (fetchOnLoad) => {
  ...
  const fetchInvoices = useCallback((companyId, onFetch) => {
    setIsLoading(true);
    const url = companyId ? `${api.companyInvoice}?userId=${companyId}` : api.invoiceList;
  ...
  const getInvoice = (endPoint, data, viewPdf = true) => {
    ...
    return axios.post(endPoint, { ...data, userId: user.userId }, { responseType: 'blob' })
      .then(data => {
        if (viewPdf) {
  ...
  const saveBills = ({ bills }, invoiceNo, onSave) => {
    ...
    billData.forEach(({
      outstandingCost, outstandingTax, paidTotal, paidDate, billId, paidCost, paidTax, method
    }) => {
      outstandingTotal += outstandingCost + outstandingTax;
    ...
    const reqData = {
      invoiceNo,
      status: outstandingTotal === allPaidTotal ? invoiceStatus.completed : invoiceStatus.outstanding,
      regBy: user.userId,
    ...
    axios.post(`${api.payment}/submit`, { ...reqData })
      .then(() => {
        onSave(bills);
      })
      .finally(() => {
  ...
  const checkBulkIssue = useCallback((onCheck) => {
    ...
    axios.get(`${api.invoiceBulkCheck}`)
      .then((resp) => {
        onCheck(resp.data);
      })
      .finally(() => {
  ...
  const issuePreviewAllInvoices = useCallback(() => {
    ...
    axios.post(`${api.invoicebulkIssue}`, { userId: user.userId, issueType: 1 }, { responseType: 'blob' })
      .then(data => {
        const file = new Blob([data.data], {type: 'application/pdf'});
        const fileURL = URL.createObjectURL(file);
        window.open(fileURL);
